...
public class GetDoctorPatientSearchByDoctorIdPgeNo {
	/*
	 * static Auth_Token auth = new Auth_Token(); static String accessToken
	 */
	static GenerateTokenDoctorUser tg = new GenerateTokenDoctorUser();
	static String accessToken = tg.getAccessToken();
	
	@Test(dataProvider="credetials")
	public static void getDoctorPatientSearchByDoctorIdPgeNo() {
		 
		Response response =

				given().header("Authorization", "Bearer " + accessToken).contentType(ContentType.JSON)
						.queryParam("doctorId", "467")
						.queryParam("PageNumber", "1")
						.log().all()
						.get(DoctorEndPoints.DoctorPatientSearchByDoctorIdPgeNo);
