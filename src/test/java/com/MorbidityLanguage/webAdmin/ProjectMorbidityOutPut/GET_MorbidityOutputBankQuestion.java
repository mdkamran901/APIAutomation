...
public class GET_MorbidityOutputBankQuestion {
	
	static TokenGenerateWeb tg = new TokenGenerateWeb();
	static String accessToken = tg.getAccessToken();

	@Test
	public static void get_MorbidityOutputBankQuestion() {
		Response response =

				given().header("Authorization", "Bearer " + accessToken).contentType(ContentType.JSON)
				.queryParam("morbidityoutputbankId", "18")
				.queryParam("morbidityoutputsectioneId", "35")
				//.queryParam("question", "test")
				//.queryParam("userid", "103")
				//.queryParam("rolename", "SuperAdmin")
						.log().all()
						.get(SuperAdminEndPoints.GET_MorbidityOutputBankQuestion);
