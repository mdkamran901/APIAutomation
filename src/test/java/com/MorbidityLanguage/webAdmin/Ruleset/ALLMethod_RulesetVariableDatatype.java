...
public class ALLMethod_RulesetVariableDatatype {
	...
	public static void post_RulesetVariable() {
		...
		String jsonBody ="{\r\n"
				+ "  \"ruleset_variable_datatype\": \"Integer123\",\r\n"
				+ "  \"ruleset_variable_datatype_key\": \"Testing1234\",\r\n"
				+ "  \"is_active\": true,\r\n"
				+ "  \"created_by\": 103,\r\n"
				+ "  \"created_on\": \"2022-07-27T14:11:47.000Z\",\r\n"
				+ "  \"is_deleted\": false\r\n"
				+ "}";
		Response response =
				...
				.body(jsonBody)
				        .log().all()
						.post(SuperAdminEndPoints.ALLMethod_RulesetVariableDataType);
	...
	public static void get_RulesetVariable() {
		Response response =
				...
				//.queryParam("userid", "103")
						.log().all()
						.get(SuperAdminEndPoints.ALLMethod_RulesetVariableDataType);

		response.prettyPrint();
		System.out.println("status code is \n" + response.statusCode());
		//System.out.println(response.getBody().asString());
		String bodyAsString = response.getBody().asString();
		Assert.assertEquals(bodyAsString.contains("ruleset_variable_datatype_id"), true 
				/*Actual Value*/, "Response body contains ruleset_variable_datatype_id");
	...
	public static void update_RulesetVariable() {
			...
			String jsonBody ="{\r\n"
					+ "  \"ruleset_variable_datatype_id\": 16,\r\n"
					+ "  \"ruleset_variable_datatype\": \"Integer123\",\r\n"
					+ "  \"ruleset_variable_datatype_key\": \"Testing Kmaran\",\r\n"
					+ "  \"is_active\": true,\r\n"
					+ "  \"created_by\": 103,\r\n"
					+ "  \"created_on\": \"2022-07-27T14:11:47\",\r\n"
					+ "  \"is_deleted\": false,\r\n"
					+ "  \"updated_by\": 103,\r\n"
					+ "  \"updated_on\": \"2022-07-27T14:12:45.000Z\",\r\n"
					+ "  \"is_dynamic\": true\r\n"
					+ "}";
			Response response =
					...
					.body(jsonBody)
					        .log().all()
							.put(SuperAdminEndPoints.ALLMethod_RulesetVariableDataType);

			response.prettyPrint();
			System.out.println("status code is \n" + response.statusCode());
			Assert.assertEquals(response.statusCode(), 200);
			Assert.assertEquals(response.getBody().path("statusCode"), "U0001");
			Assert.assertEquals(response.getBody().path("description"),"Record is updated in mw_mst_ruleset_variable_datatype table successfully");
	}
//@Test(priority=4)
	public static void delete_RulesetVariable() {
			...
			Response response =
					...
					.pathParam("morbidity_bank_id", "97")
					        .log().all()
							.delete(SuperAdminEndPoints.ALLMethod_RulesetVariableDataType);
